...
class ShopifyWebhookService
{
    ...
    public function buildOrdersUpdateWebhookUrl(): string
    {
        return rtrim($this->publicAppUrl(), '/')
...
GRAPHQL,
    ...
    public function ensureOrdersUpdateWebhook(Shop $shop): void
    {
        \Log::info('INSIDE ORDERS UPDATE WEBHOOK FUNCTION');

        $targetUrl = $this->buildOrdersUpdateWebhookUrl();

        $existingWebhook = $this->findOrdersUpdateWebhook($shop, $targetUrl);
...
GRAPHQL,
        ...
        if (!empty($errors)) {
            ...
            throw new RuntimeException(
                $message !== ''
                    ? $message
                    : 'Unable to create Shopify orders update webhook subscription.'
            );
        }
    }
...
GRAPHQL
    ...
    private function findOrdersUpdateWebhook(Shop $shop, string $targetUrl): ?array
    {
        $response = $this->graphQl(
            $shop,
            <<<'GRAPHQL'

...
Route::prefix('admin')->middleware([\App\Http\Middleware\VerifyAdminRequest::class, \App\Http\Middleware\EnsureAdminAuthenticated::class])->group(function () {
        ...
        Route::post('/notification-settings', [NotificationController::class, 'saveSettings'])->name('admin.notification.settings.save');
        Route::post('/trial-ending-notify', [NotificationController::class, 'sendTrialEndingNotifications'] )->name('admin.trial.ending.notify');
        Route::post('/notification/{id}/read', [NotificationController::class, 'markAdminNotificationRead'])->name('admin.notification.read');
});
